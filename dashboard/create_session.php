...
if (isset($_POST['envoyer'])) {
    ...
    // Assurez-vous que le matricule est correctement récupéré du formulaire
    $matricule = isset($_POST['matricule']) ? mysqli_real_escape_string($conn, $_POST['matricule']) : null;
    
    if (!empty($matricule)) {
        $matriculeQuery = "SELECT FONCTION FROM emp WHERE matricule = '$matricule' LIMIT 1";
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            // Gérez ici le cas où le matricule n'est pas trouvé
            $fonction = 'Fonction inconnue';
        }
    } else {
     // Si le matricule est vide, utilisez la logique basée sur le formulaire
        // Récupérer la valeur de "Fonction" ou du champ texte "Autre"
        if ($_POST['fonction'] === 'autre') {
            $fonction = mysqli_real_escape_string($conn, $_POST['autre']);
        } else {
...
$fonctionQuery = mysqli_query($conn, "SELECT FONCTION FROM fonction ACS ORDER BY FONCTION ASC;");
...
<body>
        ...
        <div id="page-content-wrapper">
            ...
            <nav class="navbar navbar-expand-lg navbar-light bg-light border-bottom">
                ...
                <div class="container-fluid" style="background-color:white;margin-top:3%; margin-left:1%;"><br>
                    <h2 style="text-align:center;font-familly:Dax;">Creation d'une Nouvelle Session</h2>
                    <br>
                    <form method="post" name="inscription">
                        ...
                        <div style="margin-left: 1em;width:63%;">
                            <label>Utilisateur Déja Existé:</label>
                        ...
                        <div id="divfonction">
                            ...
                            <div style="display: none;" id="autre">
                                <div style="margin-left: 1em;" >
                                    <label>Autre :</label>
                                    <input type="text" name="autre" id="input">
                                </div>
                            </div>
                        </div>
                        <div style="margin-left: 1em;width:63%;">
                        ...
                        <div  id="divOption1" style="display: none;">
                            <div style="margin-left: 1.5em;">
                                ...
                                <input type="time" id="temps" name="tempsT">
                        ...
                        <div  id="divOption2" style="display: none;">
                            <div style="margin-left: 1em;">
                                ...
                                <input type="time" id="temps1" name="temps1">
                            </div>
                            <div style="margin-left: 1em;">
                                <label>Temps Partie 2 :</label>
                                <input type="time" id="temps2" name="temps2">
                            </div>
                            <div style="margin-left: 1em;">
                                <label>Temps Partie 3 :</label>
                                <input type="time" id="temps3" name="temps3">
                            </div>
                        </div>
                        <div style="margin-left: 1em;">
                            <label>Email :</label>
                        ...
                        <div style="margin-left: 1em;">
                            <label>Message :</label>
                            <textarea name="message" id="messageTextArea" cols="30" rows="10"></textarea> <br>
                        </div>
    ...
    listItems.forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            const existSelect = document.getElementById('exist');
            const divmatricule = document.getElementById("divmatricule");
            ...
            divmatricule.style.display = 'none';
            divfonction.style.display = 'none';

            existSelect.addEventListener('change', function() {
            const selectedValue = existSelect.value;

            if (selectedValue === 'Oui') {
                // Si "Oui" est sélectionné, afficher le champ "Matricule" et masquer le champ "Fonction"
                divmatricule.style.display = 'block';
                divfonction.style.display = 'none';
                
            } else if (selectedValue === 'Non') {
                // Si "Non" est sélectionné, afficher le champ "Fonction" et masquer le champ "Matricule"
                divmatricule.style.display = 'none';
                divfonction.style.display = 'block';
                
            } else {
                // Gérer le cas où aucune option n'est sélectionnée
                alert('Veuillez sélectionner une option (Oui ou Non) dans la liste.');
            }
        });
            ...
            function updateMessageWithSessionID(message, link, sessionID) {
                ...
                messageTextArea.textContent = `${message}<br>Lien du test : ${link}?session_id=${sessionID}`;
                // messageTextArea.innerHTML = `${message}<br>Lien du test : <a href="${link}?session_id=${sessionID}" target="_blank">Cliquez ici pour passer le test</a>`;
                // const testLink = document.createElement('a');
                // testLink.href = `${link}?session_id=${sessionID}`;
                // testLink.textContent = 'Cliquez ici pour passer le test';

                // messageTextArea.innerHTML = ''; // Videz le contenu existant de messageTextArea
                // messageTextArea.appendChild(testLink);

            }
//             function updateMessageWithSessionID(message, link, sessionID) {
//     messageTextArea.innerHTML = ${message}<br>Lien du test : <a href="${link}?session_id=${sessionID}">${link}?session_id=${sessionID}</a>;
// }
        });
