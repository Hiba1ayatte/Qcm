...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    for ($questionNumber = 1; $questionNumber <= 25; $questionNumber++) {
        ...
        if (isset($userResponses[$questionNumber])) {
            ...
            $sql = "SELECT Reponse1 FROM test1 WHERE id = $questionNumber";
    ...
    $stmt = $conn->prepare("SELECT * FROM noteq1 WHERE cin = ? AND nom = ? AND prenom = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $requet = $conn->prepare("UPDATE noteq1 SET score = ? WHERE nom = ? AND prenom = ?");
        $requet->bind_param("sss",  $score, $nom , $prenom);
        $requet->execute();
        header("Location: process-quiz2.php");
        exit();
    }

    // Store the score in the database
    $sql = "INSERT INTO noteq1 (cin, nom, prenom, score) VALUES (?, ?, ?, ?)";
