...
$sql = "SELECT matricule ,fonction,test , temps_part1 ,temps_part2, temps_part3 FROM session WHERE id = $sessionID";
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    $test = $row['test'];
    $temps1 = $row['temps_part1'];
    $temps2 = $row['temps_part2'];
    $temps3 = $row['temps_part3'];
    ...
    // Convertir les temps de format "00:00" en secondes
    $temps1Parts = explode(':', $temps1);
    $temps2Parts = explode(':', $temps2);
    $temps3Parts = explode(':', $temps3);

    $temps1EnSecondes = intval($temps1Parts[0]) * 60 + intval($temps1Parts[1]);
    $temps2EnSecondes = intval($temps2Parts[0]) * 60 + intval($temps2Parts[1]);
    $temps3EnSecondes = intval($temps3Parts[0]) * 60 + intval($temps3Parts[1]);

    // Calculer le temps total en secondes
    $tempsTotalEnSecondes = $temps1EnSecondes + $temps2EnSecondes + $temps3EnSecondes;

    // Convertir le temps total en minutes et secondes
    $minutesTotal = floor($tempsTotalEnSecondes / 60);
    $secondesTotal = $tempsTotalEnSecondes % 60;
    
    // Afficher le temps total dans le format "mm:ss"
    $temps = sprintf("%02d:%02d", $minutesTotal, $secondesTotal);
    // echo $temps;
...
<div class="formulaire">
		<form method="post" action="verifier_candidatQ1.php">
            ...
            <?php
                ...
                <div style="display:flex">
                    ...
                    <div id="pre">
                        <label >Socité :</label>
                        <input type="text" name="matricule" value="<?php echo !empty($societe) ? $societe : ''; ?>" readonly><br>
                    </div>
                </div>
                <div style="display:flex">
                    <div>
                        <label>Direction :</label>
                        <input type="text" name="matricule" value="<?php echo !empty($direction) ? $direction : ''; ?>" readonly><br>
                    </div>
                    <div id="mail">
                        <label>Service :</label>
                        <input type="text" name="matricule" value="<?php echo !empty($service) ? $service : ''; ?>" readonly><br>
                    </div>
                </div>
                <?php
